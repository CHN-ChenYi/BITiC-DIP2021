...
inline uint8_t OtsuMethod(const unsigned histogram[256], const int pixel_count) {
  ...
  for (int i = 0; i < 256 && probability[i] != 1; i++) {
    if (probability[i] != 0) {
      between = pow(mean[255] * probability[i] - mean[i], 2) / (probability[i] * (1.0 - probability[i]));
